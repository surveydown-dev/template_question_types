...
Each option is a clickable image card. Provide an `image` vector (one image path or URL per option, in the same order as `option`). Image paths resolve against the survey's `images/` folder. When the `option` vector is **named**, the names are shown as text captions beneath each image.

```{r}
sd_question(
  id     = 'apple_image',
  type   = 'mc_image',
  label  = "Which apple variety looks most appealing?",
  option = c(
    "Fuji"       = "fuji",
    "Pink Lady"  = "pink_lady",
...
Same as `mc_image` but multiple cards can be selected. Here the `option` vector is **unnamed**, so the images are shown with no captions (the values are still stored in the data).

```{r}
sd_question(
  id     = 'apple_buy',
  type   = 'mc_multiple_image',
  label  = "Which apples would you buy (select all that apply)?",
  option = c(
    "Fuji"       = "fuji",
    "Pink Lady"  = "pink_lady",
    "Honeycrisp" = "honeycrisp"
...
sd_question(
  ...
  label  = "Which features matter to you for each vehicle type (select all that apply)?",
  row    = c(
